...
cron.schedule('*/2 * * * *', async () => {
    ...
    db.query(updateQuery, [nextTime], async (err, result) => {
        ...
        if (result.affectedRows === 0) {
            ...
            console.log('time update  successfully');
                ...
                for (const entry of resultArray) {
                    ...
                    getWalletAmount(user_id)
                        .then(currentAmount => {
                            ...
                            updateWalletAmount(user_id, newAmount)
                                .then(updatedWallet => {
...
function updateWalletAmount(user_id, newAmount) {
    return new Promise((resolve, reject) => {
...
app.get('/user_wallet', (req, res) => {
    const user_id = req.query.user_id; // Extract the user_id from the query parameters
    console.log("user_id", user_id); // Log the extracted user_id
    const query = 'SELECT wallet FROM wallet WHERE user_id = ?';
...
app.get('/', (req, res) => {
    ...
    var formattedCurrentTime = currentTime.toLocaleTimeString('en-US', options);
    var formattedNextTime = nextTime.toLocaleTimeString('en-US', options);
    res.send({ message: 'today now', formattedCurrentTime, formattedNextTime });
